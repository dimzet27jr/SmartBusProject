...
namespace SmartBusProject
{
    public partial class EnergyForm : Form
    {
        ...
        private bool acWarningShown = false;
        ...
        private void EnergyTimer_Tick(object sender, EventArgs e)
        {
            ...
            if (wasAcOpen && !acWarningShown && energySystem.EnergyLevel/30 < 20)
            {
                acWarningShown = true;
            ...
            if (energySystem.EnergyLevel / 30 >= 20)
            {
                energySystem.BusEngineConsumption = 15;
                energySystem.BusScreenConsumption = 35;
                lowEnergyWarningShown = false;
                acWarningShown = false;
                criticalEnergyWarningShown = false;
                emptyBatteryWarning = false;
            }
        ...
        private void UpdateLabels()
        {
            ...
            else if (energySystem.Weather == WeatherType.Rainy)
            {
                ...
                if (pbxPanelControls.Enabled && !panelWarningShown)
                {
                    panelWarningShown = true;
                    MessageBox.Show("Τα φωτοβολταϊκά δεν μπορούν να ανοίξουν!\n Ο καιρός είναι βροχερός.", "Ξεφόρτιστο Λεωφορείο", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }               
